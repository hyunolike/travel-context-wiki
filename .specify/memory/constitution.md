# Hanjeok Evidence Wiki Constitution

## Core Principles

### I. Evidence First

Every reusable claim must trace to a file under `raw/`. New service behavior starts by preserving source material or a deterministic fixture before creating canonical synthesis.

### II. Deterministic Recommendation Boundary

The LLM wiki explains Hanjeok recommendations; it does not decide ranking, reorder courses, invent attractions, or override backend facts.
...
OpenAPI usage constraints, raw/derived data separation, local-server storage obligations, and operation-account checks are first-class product requirements.

## Repository Constraints

- Markdown and Git are the durable storage layer.
- `raw/` source snapshots are immutable after capture.
- API keys, tokens, user travel history, and private location records are never committed.
...
This constitution supersedes ad hoc note-taking practices in this repository. Changes to the evidence model, canonical directories, or LLM recommendation boundary require an explicit decision page under `decisions/`.
