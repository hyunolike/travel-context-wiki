...
The current Hanjeok policy combines:
...
If the related-attraction API does not provide a numeric relatedness score, Hanjeok may replace `w1` with a binary "is related" weight or remove the term.

## Runtime Boundary

The backend owns this score. [[evidence-backed-course-explanation]] may explain the resulting score factors, but it must not recalculate or override the ranked list.
